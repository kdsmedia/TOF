...
func _input(event):
    ...
    if Input.is_action_pressed('ui_cancel') && (not (event is InputEventKey) || not event.is_echo()):
...
func load_map(template_name, workshop_file_name = false, load_saved_state = false, is_remote = false, post_load_object = null, post_load_method = null):
    ...
    self.load_map_for_real(template_name, workshop_file_name, load_saved_state, is_remote, post_load_object, post_load_method)
...
func load_map_for_real(template_name, workshop_file_name = false, load_saved_state = false, is_remote = false, post_load_object = null, post_load_method = null):
    ...
    current_map.get_node('terrain').tileset = self.bag.tileset_handler.available_tilesets[self.main_tileset]
    ...
    if post_load_object != null:
        ...
        post_load_object.call(post_load_method)
...
func write_settings_to_file():
    ...
    file.store_string(JSON.print(self.settings))
    file.close()
...
func _ready():
    ...
    if err == OK:
        ...
        if admob.has_user_signal("interstitial_loaded"):
            admob.connect("interstitial_loaded", self, "_on_admob_interstitial_loaded")
        if admob.has_user_signal("rewarded_video_loaded"):
